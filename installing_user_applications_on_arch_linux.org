...
To download, build and install =yay=, you'll need =git= (~sudo pacman
-S git~). Clone the official =yay= /git/ repository:
...
happens when necessary — you'll then be prompted for a password then.

**** yay-bin

Now that you have =yay= on your machine, it is possible, through it,
to install =yay-bin= and get rid of =go= (Core compiler tools for the
Go programming language), a dependency required to build =yay=.

Install =yay-bin= by running ~yay -S yay-bin~.

Then, to verify that =go= is an orphaned package, execute ~pacman
-Qdtq~ (~man pacman~ for the details).

Now uninstall go with ~sudo pacman -R go~.
